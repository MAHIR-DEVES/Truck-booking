...
const avatarImages: StaticImageData[] = [img1, img2, img3, img2, img3];
...
export default function Testimonials() {
  ...
  const testimonials: Testimonial[] = t
    ...
    .map((item: TestimonialItem, index: number) => ({
      id: index + 1,
      quote: item.quote,
      author: item.author,
      role: item.role,
            ...
            .map((testimonial: Testimonial, idx: number) => (
              ...
              >
                <div className="text-center bg-card p-6 rounded-lg shadow-md">
                  <p className="text-card-foreground mb-8 text-sm md:text-base leading-relaxed">
                    {testimonial.quote}
                  </p>
        ...
        <div className="flex justify-center items-center gap-2 mt-4">
          {testimonials.map((_: Testimonial, index: number) => (
            <button
              ...
              aria-label={`${t('testimonials.navigation.viewSet')} ${
                index + 1
              }`}
            />
          ))}
